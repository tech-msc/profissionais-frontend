...
export class ProfissionalService {
  ...
  profissionalPorID(id: number): Observable<Profissional> {
    return this.http.get<Profissional>(`${API}/profissional/${id}`).pipe(
  ...
  profissionalEditar(
    ...
    return this.http.put(`${API}/profissional/${id}`, profissional).pipe(
      map((response) => response),
  ...
  profissionalNovo(profissional: Profissional): Observable<Profissional> {
    return this.http.post(`${API}/profissional`, profissional).pipe(
      map((response) => response),
      catchError(ErrorHandler.handleError('new profissional'))
    )
  }

  profissionalRemover(id: number): Observable<any> {
    return this.http.delete(`${API}/profissional/${id}`).pipe(
      map((response) => response),
      catchError(ErrorHandler.handleError('remove profissional'))
    )
  }
}
